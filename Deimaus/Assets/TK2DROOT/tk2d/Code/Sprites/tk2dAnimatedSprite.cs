...
public class tk2dAnimatedSprite : tk2dSprite
{
	...
	// Warps the current active frame to the time specified. Ensure that time doesn't exceed the number of frames
	// Will warp silently otherwise
	void WarpClipToTime(tk2dSpriteAnimationClip clip, float time)
	{
		clipTime = time;
//		int myFrameId = (int)clipTime;
	...
	public void Play(tk2dSpriteAnimationClip clip, float clipStartTime)
	{
		if (clip != null)
		{
			...
			{
				WarpClipToTime(currentClip, 0.0f);
				state &= ~State.Playing;
			}
			else if (currentClip.wrapMode == tk2dSpriteAnimationClip.WrapMode.RandomFrame || currentClip.wrapMode == tk2dSpriteAnimationClip.WrapMode.RandomLoop)
			{
				...
				WarpClipToTime(currentClip, rnd * currentClip.fps);
			...
			else
			{
				...
				{
					// warp to last frame
					WarpClipToTime(currentClip, currentClip.fps * (currentClip.frames.Length - 1));
					state &= ~State.Playing;
				}
				else
				{
					WarpClipToTime(currentClip, time);
