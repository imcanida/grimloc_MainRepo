...
public class AIFollow : MonoBehaviour {
	...
	public void Update () {
		...
		forwardDir = new Vector3(Mathf.Clamp(forwardDir.x, -1, 1), 0, Mathf.Clamp(forwardDir.z, -1, 1) );
		
		if (navmeshController != null) {
			navmeshController.SimpleMove (tr.position,forwardDir);
		} else {
			controller.Move(forwardDir*Time.deltaTime*speed);
		}
		
		//Can Control animation states from forwardDir as well
		
		tr.localPosition = new Vector3(tr.localPosition.x, baseYLocation, tr.localPosition.z);
		tr.localPosition += new Vector3(0, tr.localPosition.z*-0.18f, 0);

	}
	public float baseYLocation = -6f;
	
	public bool AnimationDirectionHandling(Vector3 direction)
	{
